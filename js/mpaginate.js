...
mpaginate.view = function(ctrl) {
    return [
        m('ul.list-inline.text-center', [
            ctrl.startPage() > 0 ? m('li', [ m('a.btn.btn-default.btn-xs', {onclick: ctrl.prevPage}, '<<' )]) : null,
            ctrl.numPages().length < 2 ? null : ctrl.numPages().slice(ctrl.startPage(), ctrl.endPage()).map(function(page, index) {
                return m('li', [
                    m('a', {
                        ...
                        class: ctrl.currentPage() == page ? 'btn btn-default btn-xs active' : 'btn btn-default btn-xs'}, page)
                    ])
                }),
            ctrl.numPages().length > ctrl.endPage() ? m('li', [ m('a.btn.btn-default.btn-xs', {onclick: ctrl.nextPage}, '>>')]) : null
        ])
    ]
}
